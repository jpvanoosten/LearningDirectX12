...
Tutorial6::Tutorial6( const std::wstring& name, int width, int height, bool vSync )
...
, m_Height( height )
, m_IsLoading( true )
, m_RenderTechnique( RenderTechnique::Raytrace )
, m_FPS( 0.0f )
{
#if _DEBUG
    ...
    m_Window->KeyReleased += KeyboardEvent::slot( &Tutorial6::OnKeyReleased, this );
    m_Window->MouseMoved += MouseMotionEvent::slot( &Tutorial6::OnMouseMoved, this );
}
...
bool Tutorial6::LoadingProgress( float loadingProgress )
{
...
bool Tutorial6::LoadScene( const std::wstring& sceneFile )
{
    ...
    auto scene    = commandList->LoadSceneFromFile( sceneFile, std::bind( &Tutorial6::LoadingProgress, this, _1 ) );
...
void Tutorial6::LoadContent()
{
    m_Device = Device::Create();
    m_Logger->info( L"Device created: {}", m_Device->GetDescription() );
    m_Logger->info( L"Raytracing supported: {}", m_Device->IsRaytracingSupported() );

    if ( !m_Device->IsRaytracingSupported() )
    {
        // Make sure we don't use the ray tracing technique if ray tracing isn't supported.
        m_RenderTechnique = RenderTechnique::Raster;
    }
    ...
    m_LoadingTask = std::async( std::launch::async, std::bind( &Tutorial6::LoadScene, this,
...
void Tutorial6::UnloadContent() {}

void Tutorial6::OnUpdate( UpdateEventArgs& e )
{
...
void Tutorial6::OnResize( ResizeEventArgs& e )
{
...
void Tutorial6::RenderRaster( CommandList& commandList, const RenderTarget& renderTarget )
{
...
void Tutorial6::RenderRaytrace( CommandList& commandList, const RenderTarget& renderTarget )
{
    // Clear the render targets.
    {
        FLOAT clearColor[] = { 0.4f, 0.6f, 0.9f, 1.0f };
...
void Tutorial6::OnRender()
{
    ...
    else
    {
        ...
        case RenderTechnique::Raster:
            RenderRaster( *commandList, m_RenderTarget );
            break;
        case RenderTechnique::Raytrace:
            RenderRaytrace( *commandList, m_RenderTarget );
            break;
        default:
            break;
        }
...
void Tutorial6::OnKeyPressed( KeyEventArgs& e )
{
...
void Tutorial6::OnKeyReleased( KeyEventArgs& e )
{
    if ( !ImGui::GetIO().WantCaptureKeyboard )
    {
        switch ( e.Key )
...
void Tutorial6::OnMouseMoved( MouseMotionEventArgs& e )
{
    if ( !ImGui::GetIO().WantCaptureMouse ) {}
}

void Tutorial6::OnDPIScaleChanged( DPIScaleEventArgs& e )
{
...
void Tutorial6::OnGUI( const std::shared_ptr<CommandList>& commandList, const RenderTarget& renderTarget )
{
    m_GUI->NewFrame();
    ...
    if ( ImGui::BeginMainMenuBar() )
    {
        ...
        if ( ImGui::BeginMenu( "Options" ) )
        {
            ...
            // Enable/disable raytracing (if supported)
            bool raytraceSupported = m_Device->IsRaytracingSupported();
            bool raytrace = raytraceSupported && m_RenderTechnique == RenderTechnique::Raytrace;
            if ( !raytraceSupported ) 
            {
                ImGui::PushItemFlag( ImGuiItemFlags_Disabled, true );
                ImGui::PushStyleVar( ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f );
            }
            if ( ImGui::MenuItem( "Raytracing", nullptr, &raytrace ) )
            {
                m_RenderTechnique = ( raytraceSupported && raytrace ) ? RenderTechnique::Raytrace : RenderTechnique::Raster;
            }
            if ( !raytraceSupported )
            {
                ImGui::PopItemFlag();
            ...
            if ( SUCCEEDED( pFileOpen->Show( m_Window->GetWindowHandle() ) ) )
            {
                ...
                if ( SUCCEEDED( pFileOpen->GetResult( &pItem ) ) )
                {
                    ...
                    if ( SUCCEEDED( pItem->GetDisplayName( SIGDN_FILESYSPATH, &pszFilePath ) ) )
                    {
                        ...
                        m_LoadingTask =
                            std::async( std::launch::async, std::bind( &Tutorial6::LoadScene, this, pszFilePath ) );
